...
class Bacteria(object):
    ...
    genome : list
        хранит в себе геном клетки в первой ячейке хранится
        радиус зрения клетки, во второй ячейке хранится list с шансами пойти в
        одно из возможных направалений [up, down, left, right], в третьей
        ячейке хранится ген отвечающий зато чем питаються клетки '0'-растения,
        а '1'-мясо.
    position : list
        хранит в себе информации о позиции клетки на поле (x, y)
    memory : list
        хранит в себе информацию об пищи, которую они могут есть
    memory_enemy : list
        хранит в себе информацию об объектах не относящихся в пищи
    delete : bool
        жива она или мертва клетка
    colour : tuple
    ...
    __move_rules(width, height, objects_positions, start=False)
        Функция отвчечающая за передвежение клетки, когда нет пищи в поле
        зрения или когда клетка не может пройти к еде.
    __die(age=40)
        Функция отпределяет умерла ли клетка от старости
    ...
    def __inspect(self, width, height, meat_positions, food_positions,
                  cell_positions):
        """Функция отвечающая за зрение клетки. В данной функции запомнимается
    ...
    def __move_rules(self, width, height, objects_positions, start=False):
        """Функция отвечает движение клетки, когда она не видит еду, если
    ...
    def __die(self, age=40):
        """Функция отвечает за смерть клетки от стрости.
        """
        if self.age > age:
            self.delete = True
            self.colour = RED
    ...
    def check_energy(self):
        """Функция отвечает за смерть клетки от окончание энергии
        """
        if self.energy <= 0:
            self.delete = True
            self.colour = RED
    ...
    def division(self, width, height, root):
        """Функция отвечающая за деление клетки
            ...
            Если нет место для деления клтеки, то клетка не будет делиться
