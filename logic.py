...
DATA = {}  # Словарь, в котором хранятся после сохраненения или загрузки
...
GENERATIONS = [1]  # Лист, в котором записывается среднее поколения клетки
...
MEAT_POSITIONS = []  # Лист, в котором хранятся все позиции мясо
FOOD_POSITIONS = []  # Лист, в котором хранятся все позиции еды
CELL_POSITIONS = []  # Лист, в котором хранятся все позиции клеток
OBJECTS_POSITIONS = []  # Лист, в котором хранятся все позиции все объектов
...
def ter():
    """Функция отрисовки окна
        ...
        buttonCallback - функция получает и устонавливает количество еды
        button_call - функция остонавливает симуляция
        start - запускает симуляция
        restart - перезапускает симуляция и ждем начала симуляции
    """
...
def __delete_food(root, foods, objects_positions, food_positions):
    """Удавляет еды, которую клетки съели
    """
    delete_food = []
...
def __delete_meat(root, meats, meat_positions, objects_positions):
    """Удавляет мясо, которую клетки съели
    """
    delete_meat = []
...
def __move_cell(root, cells, meats, age, objects_positions, food_positions,
    ...
    """Функция отвечающая за движение клетки, а также удаление клеток, которые
       ...
       Возвращает лист состоящий из генов зрения клеток, среднее значения
...
def __division(root, width, height, cells, meats, meat_positions,
    ...
    """Функция отвечающая за деление клетки, добовление новых клеток и мяса в
       листы
    """
    new_cells = []
    delete_cells = []
...
def start_simulation():
    """Функция отвечаающая за обработку данных на каждом шаге симуляции
    """
    global NUMBER_FOOD, FLAG, res, DATA, HEIGHT, WIDTH, HISTORY_FOODS, FOODS
    ...
    """Основной цикл, в котором происходит запись данных для посторения 
...
def start_program():
    ...
    start_simulation()
    """Если не была нажата кнопка рестарта, то вызывается функции постройки 
